...
NetGuard is intentionally opt-in because it changes host `iptables` rules.
Start it only when required with `docker compose --profile netguard up --build`.
...
standalone transparent TCP proxy for traffic generated by its Linux host.
It currently handles local IPv4 and IPv6 TCP ports 80/443; it is not a LAN
gateway, UDP/QUIC inspector, or Android VPN. It inspects each
connection's hostname (SNI), TLS fingerprint (JA3), and
...
Isolation Forest detector and a validated Random Forest/Keras payload
ensemble for inspectable plaintext traffic.

```bash
sudo uv run shieldnet netguard
```

- Proxy: `:8888` (default) — transparent TCP interception
- Dashboard: `:8080` — live connection stats and decisions
- Uses an owned, idempotent `iptables` chain and requires root or equivalent
  network capabilities. Graceful shutdown removes its rules.
- URL-model output is review/telemetry evidence, not block authority. Hard
  blocking requires a deterministic signature or independent verified
  evidence; there is no built-in domain allowlist.
- URL lexical rules are treated as correlated with the URL model, and anomaly
  output is monitoring-only for enforcement corroboration.
- A supervised pre-relay payload ensemble (`netguard-train-payload` and
  `netguard-train-deep-payload`, trained from IoT-23 captures) can raise risk
  from the first client bytes before forwarding. Its two correlated models
  (a Random Forest and a compact regularized Keras classifier) count as one
  evidence source. TLS ClientHello bytes are excluded from both training and
  inference because
  encrypted application content is not visible there.
- `/api/status` reports degraded protection unless the proxy, enforcement
  chains, ShieldNet URL model, Isolation Forest, and both payload artifacts
  are loaded.

It has no Android/Shizuku/ADB dependency — it is a plain Linux service.

Every CLI flag, decision threshold, port/TLD/JA3 list, and all 42
feature-vector slots are documented in
**[NETGUARD_PARAMETERS.md](NETGUARD_PARAMETERS.md)**.
...
| `shieldnet netguard-train-normal` | Retrain the anomaly detector from collected known-normal traffic |
| `shieldnet netguard-train-payload` | Train the Random Forest initial-payload model |
| `shieldnet netguard-train-deep-payload` | Train the compact Keras initial-payload model and metadata |
...
├── requirements.txt      Bounded runtime dependencies (mirrors pyproject.toml)
├── Dockerfile / docker-compose.yml
├── BRAIN.md              Full project architecture, explained
├── MODEL_BRAIN.md        The neural network's internals, explained
├── NETGUARD_PARAMETERS.md  NetGuard CLI flags, thresholds, and 42 features
